...
if ! grep -q "^GLUETUN_API_KEY=" "$ENV_FILE" 2>/dev/null; then
    echo "GLUETUN_API_KEY=" >> "$ENV_FILE"
fi
if is_placeholder "GLUETUN_API_KEY"; then
    set_env "GLUETUN_API_KEY" "$(tr -dc 'A-Za-z0-9' </dev/urandom | head -c 32)"
    ok "Gluetun API key generated"
else
    skip "GLUETUN_API_KEY already set"
fi
# (Re)write the auth config so it always matches the key in .env
cat > "$STACK_DIR/config/gluetun/config.toml" <<EOF
# Gluetun control-server auth — generated by install.sh, matches GLUETUN_API_KEY in .env
[[roles]]
name = "homepage"
routes = ["GET /v1/publicip/ip", "GET /v1/vpn/status", "GET /v1/openvpn/portforwarded"]
auth = "apikey"
apikey = "$(env_val GLUETUN_API_KEY)"
EOF
ok "Gluetun control-server auth config written"
set -o allexport; source "$ENV_FILE"; set +o allexport
